...
<style>
  .shop-page { padding-top: calc(var(--nav-height, 72px) + 2rem); }
  .shop-hero { background:#0a0a0a; color:#fff; padding:3rem 0 2.2rem; text-align:center; }
  .shop-hero h1 { font-family: Georgia, serif; font-size: clamp(2rem, 4vw, 3.2rem); font-weight: 400; }
  .shop-hero p { color: rgba(255,255,255,.52); font-size:.9rem; margin-top:.6rem; }

  .shop-layout { display:grid; grid-template-columns:270px 1fr; gap:1.5rem; padding:2rem 0 3rem; }
  .shop-sidebar { background:#fff; border:1px solid #e8e6e2; border-radius:10px; padding:1rem; position:sticky; top:calc(72px + 1rem); }
  .f-title { font-size:.7rem; font-weight:700; letter-spacing:.16em; text-transform:uppercase; color:#7a7570; margin-bottom:.9rem; }
  .f-group { margin-bottom:1rem; }
  .f-label { display:block; font-size:.72rem; color:#7a7570; margin-bottom:.35rem; font-weight:600; }
  .f-input, .f-select { width:100%; padding:.6rem .75rem; border:1.5px solid #e8e6e2; border-radius:8px; font-size:.85rem; box-sizing:border-box; }
  .f-row { display:grid; grid-template-columns:1fr 1fr; gap:.5rem; }
  .f-actions { display:grid; grid-template-columns:1fr 1fr; gap:.5rem; }
  .btn-filter { border:none; border-radius:8px; padding:.65rem; font-size:.72rem; font-weight:700; letter-spacing:.08em; text-transform:uppercase; cursor:pointer; }
  .btn-filter.apply { background:#0a0a0a; color:#fff; }
  .btn-filter.clear { background:#fff; border:1px solid #e8e6e2; color:#7a7570; text-decoration:none; display:flex; align-items:center; justify-content:center; }
  ...
  .result-meta { font-size:.78rem; color:#7a7570; }

  .products-grid { display:grid; grid-template-columns:repeat(3, minmax(0, 1fr)); gap:1rem; }
  .p-card { background:#fff; border:1px solid #e8e6e2; border-radius:10px; overflow:hidden; }
  .p-image { aspect-ratio:3/4; display:block; background:#f4f3f1; }
  .p-image img { width:100%; height:100%; object-fit:cover; display:block; }
  .p-body { padding:.8rem; }
  .p-name { display:block; color:#0a0a0a; text-decoration:none; font-weight:600; margin-bottom:.35rem; }
  .p-price { font-size:.95rem; font-weight:700; margin-bottom:.6rem; }
  .p-actions { display:flex; gap:.45rem; }
  .btn-p { flex:1; border:none; border-radius:7px; padding:.58rem .6rem; font-size:.69rem; font-weight:700; letter-spacing:.08em; text-transform:uppercase; cursor:pointer; }
  .btn-p.cart { background:#0a0a0a; color:#fff; }
  .btn-p.wish { background:#fff; border:1px solid #e8e6e2; color:#7a7570; }
  .btn-p.wish.active { border-color:#0a0a0a; color:#0a0a0a; }

  .pagination { margin-top:1.2rem; display:flex; gap:.4rem; flex-wrap:wrap; }
  .pagination a, .pagination span { min-width:34px; text-align:center; padding:.45rem .55rem; border:1px solid #e8e6e2; border-radius:6px; font-size:.78rem; text-decoration:none; color:#4a4743; background:#fff; }
  .pagination .active { background:#0a0a0a; color:#fff; border-color:#0a0a0a; }
                ...
                <div class="p-body">
                  ...
                  <div class="p-actions">
                    <form method="POST" action="<?= url('cart/add') ?>" style="flex:1;">
                      ...
                      <button type="submit" class="btn-p cart">Add to Cart</button>
                    </form>
